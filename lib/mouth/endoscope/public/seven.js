...
Seven.prototype = {
  ...
  graph: function(opts) {
    ...
    this.xScale = d3.time.scale().domain([this.startDate, this.endDate]).range([margin, this.graphWidth - margin]);
    this.xScaleIndex = d3.time.scale().domain([this.startDate, this.endDate]).range([0, this.points - 1]);
    this.yScale = d3.scale.linear().domain([d3.min(allData.concat([0])), maxValue]).range([margin, this.graphHeight - margin]);
    ...
    svgGroup.append("svg:line") // y-axis
        ...
        .attr("y2", -1 * this.yScale(maxValue))
        .attr('stroke', '#AAA').attr('stroke-width', 1);
  ...
  showTip: function(opts) {
    ...
    var date = this.xScale.invert(opts.x)
