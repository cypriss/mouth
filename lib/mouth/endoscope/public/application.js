...
var GraphView = Backbone.View.extend({
  ...
  render: function() {
    ...
    e.css('top', m.get('position').top * 30 + 'px');
    e.css('left', m.get('position').left * 30 + 'px');
    e.css('width', m.get('position').width * 30 + 'px');
    e.css('height', m.get('position').height * 30 + 'px');
    e.css('background-color', '#FEF');
    
    e.draggable({grid: [30,30], containment: 'parent', cursor: 'move', stop: this.onDragStop});
    e.css('position', 'absolute'); // NOTE: draggable applies position: relative, which seems completely retarded.
    this.updateData();
  ...
  updateData: function() {
    ...
    $.getJSON('/graphs/' + this.model.get('id') + '/data', {poop: 1}, function(data) {
      ...
      // Mark's task: take some candlestick data and shart it into a shart.
