...
var GraphView = Backbone.View.extend({
  ...
  render: function() {
    ...
    template = (this.mode === 'front') ? '<div class="graph-header"><span></span> <a href="#" class="edit">Edit</a></div><div class="graph-body" id="graph'+this.zid+'"></div>' :
                                         '<div class="graph-header"><span></span> <a href="#" class="edit">Edit</a></div><div class="graph-back"><select class="data-source"></select> <a href="#" class="delete">Delete</a></div>';
  ...
  updateData: function() {
    ...
    $.getJSON('/graphs/' + self.model.get('id') + '/data', {poop: 1}, function(data) {
      ...
      // self.$('.graph-body').html(JSON.stringify(data));
      
      data = data[0];;
      var seven = new Seven('#graph' + self.zid, {start: 1325404800000, step: 'minute', points: 120});
      seven.graph({name: 'zz', data: data})
