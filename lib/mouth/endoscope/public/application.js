...
var GraphView = Backbone.View.extend({
  ...
  render: function() {
    ...
    template = (this.mode === 'front') ? '<div class="graph-inner"><div class="graph-header"><span></span> <a href="#" class="edit">Edit</a></div><div class="graph-body" id="graph'+this.zid+'"></div></div>' :
  ...
  updateData: function() {
    ...
    $.getJSON('/graphs/' + self.model.get('id') + '/data', {}, function(data) {
      ...
      _.each(data, function(d) {
        if (!seven) {
          seven = new Seven('#graph' + self.zid, {start: d.start_time * 1000, step: 'minute', points: d.data.length, kind: self.model.get('kind')});
        }
